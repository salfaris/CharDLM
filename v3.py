...
class BigramLanguageModel(nnx.Module):
    ...
    def generate_fast(
        ...
        def generate_step(carry, step_idx):
            ...
            idx_cond = jax.lax.dynamic_slice(
                tokens,
                (0, jnp.maximum(0, current_pos - BLOCK_SIZE + 1)),
                (B, BLOCK_SIZE)
            )
        ...
        (final_tokens, _), _ = jax.lax.scan(
            generate_step,
            (output_tokens, rngs()),  # Note: idx is (B, T) array of indices in current context. 
...
if not GENERATE_ONLY:
    ...
    context = jnp.zeros((1, 1), dtype=jnp.int32)
    # logging.info(decode(model.generate(context, max_new_tokens=500, rngs=rngs)[0].tolist()))
    logging.info(decode(model.generate_fast(context, max_new_tokens=500, rngs=rngs)[0].tolist()))

    time_elapsed = time.perf_counter() - gen_start_time
    logging.info(f"\n{time_elapsed:.2f} seconds for decode.")
